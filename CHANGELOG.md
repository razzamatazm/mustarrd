...
### Added: Record a time slot you pick yourself

**What you would notice:** You can now record a channel between two times you choose, instead of only being able to click a programme in the guide. There's a **Record time slot** button above the guide on any channel with a catchup archive, and it also appears on channels whose guide is empty — where previously you'd hit "No EPG data available" and there was simply nothing you could do. This solves two things. If your provider gives you no listings for a channel, you can still record from it. And if a programme is far longer than the bit you want — eight hours of tennis coverage when you only want the quarter-final — you can take just the couple of hours you care about instead of the whole thing.

You type a start and an end, give the recording a name (one is suggested, and you can edit it), and that's it. The times you enter are on the same clock the guide shows for that channel, so if the guide says the match starts at 2pm, you type 2pm. Unlike guide recordings, no extra minutes are added at either end — a time slot records exactly the range you asked for, and the form says so. You can go back as far as your provider's catchup archive reaches for that channel (it tells you how far that is if you overshoot), up to 14 days ahead, and any one slot can be up to 6 hours long. If the range you pick overlaps something you're already recording on that channel, you get a warning naming it and can go ahead anyway — sometimes a wide safety recording plus a tight one is exactly what you want.

A time slot that has already aired starts downloading straight away. One that hasn't aired yet waits until your provider adds it to the catchup archive and then downloads, in the same way scheduled recordings already do. It does **not** capture the live stream, and the form tells you so — this is a deliberate decision, written up in `docs/adr/0004-time-slot-recording-uses-catchup-not-live.md`. Time slots appear in Downloads and Scheduled alongside everything else and can be cancelled the same way. Download-only users can create them too.

**What changed:** A new `POST /api/time-slots` endpoint takes a channel, a start, an end and a name, and decides from the end time whether to queue a download now or create a scheduled recording that waits. Underneath, it reuses the existing catchup pipeline unchanged — the download builder never needed an EPG programme, only a start and a duration. The bounds a guide-picked programme gets for free from the provider are enforced here instead: the channel's own catchup depth backwards, 14 days forwards, and a 6-hour cap on the range. The one genuinely new piece of logic is timezone handling. Every existing time helper converts a provider timestamp into something to display; a typed time needs the reverse, and it has to account for the per-account guide offset or the provider serves back the wrong hour. The recording also carries a provider wall-clock start token derived from that offset, which guide recordings inherit from the listings and a time slot has to work out for itself.
...
**What changed:** There is no "play it directly" option for On Demand, because there is nothing to play directly: a survey of the live catalogues found movies are 94% MKV and episodes 99.8% MKV, and every single MP4 sampled had Dolby Digital audio — so no browser can decode any of it. Every On Demand preview is converted on the server, copying the picture through untouched and re-encoding only the sound. The new part is jumping around. To start halfway through a film, the converter has to be able to ask for bytes from the middle of it — and it cannot be handed your provider's address to ask with, because that address contains your username and password, and anything passed to the converter that way can be read by anyone else using the same machine. So Mustarrd passes the file along itself, over a private connection that never leaves the machine, unlocked by a random pass phrase that is thrown away when you close the preview and refused to anything not running on that same machine. Your credentials never leave the app. The preview also reads the running time from your provider's own listings, which is why the scrub bar is the length of the film from the first frame rather than growing as the conversion catches up. The fifteen-minute limit belongs to the preview as a whole, so jumping around does not hand you another fifteen minutes.
...
### Changed: Comskip now gets a safe per-recording configuration with optional ticker exclusion
...
**What changed:** Frontend only — a presentation-layer redesign of `Browse.jsx`, `EPGGrid.jsx`, `Downloads.jsx`, and `Scheduled.jsx` with new CSS modules. All data fetching, websocket progress updates, mutations, modals (download/schedule/preview/VOD), guide-offset handling, URL tab parameters, and notifications are unchanged. `?tab=upcoming` on Downloads now falls back to Active. A shared `HistoryRow` component replaced the per-page history cards, the pipeline cards replaced the stacked progress bars (the old `ProgressBar` component was removed), and the unused desktop panel-height measuring code in Browse was replaced by CSS viewport sizing. New responsive rules: the channel rail narrows and buttons go icon-only between 821–1180px, and below 820px the panels stack with the existing channel drill-down.
...
**What you would notice:** If you point the download folder and the completed folder at the same directory, restarting Mustarrd while a recording was waiting for (or in the middle of) commercial detection or transcoding used to mark that recording as completed without ever processing it, so you would end up with the raw recording, commercials and all. After this fix, the recording is picked up again after the restart and goes through commercial skip and transcoding as configured. Saving Settings with both folders set to the same path now also logs a warning so the setup is easy to spot in the Logs page.
